...
class ModelModuleRelatedOptions extends Model {
  ...
  public function getJournal2Price($product_id, &$special=false) {
	...
	public function get_related_options_set_by_poids($product_id, $options, $full_equal=false) {
		...
		if (!$this->installed() || !is_array($options) || count($options)==0 || !$this->get_product_related_options_use($product_id)) {
			return FALSE;
		}
  ...
  public function get_options_array($product_id, $only_zero=false) {
	...
	public function get_ro_prices($product_id) {
		...
		$query = $this->db->query("SELECT RO.relatedoptions_id, RO.price ro_price, ROD.price, ROD.quantity, RO.model, RO.description, RO.quantity
															FROM ".DB_PREFIX."relatedoptions RO
		...
		foreach ($query->rows as $row) {
			if (!isset($ro_prices[$row['relatedoptions_id']])) {
				$ro_prices[$row['relatedoptions_id']] = array('price'=>$row['ro_price'], 'quantity'=>$row['quantity'], 'model'=>$row['model'], 'description'=>$row['description'], 'discounts'=>array(), 'specials'=>array(), 'special'=>false);
			}
			if ($row['price']) {
